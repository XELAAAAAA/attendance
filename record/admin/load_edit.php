...
<form method = "POST" action = "edit_query.php?admin_id=<?php echo $f_edit_admin['admin_id']?>" enctype = "multipart/form-data">
	<div class  = "modal-body">
		...
		<div class = "form-group">
			...
			<input type = "password" required = "required" maxlength = "12" value = "<?php echo $f_edit_admin['password']?>" name = "password" class = "form-control" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
		</div>
		<div class = "form-group">
